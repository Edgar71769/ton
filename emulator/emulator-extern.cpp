...
const char *transaction_emulator_sbs_emulate_transaction(void *transaction_emulator, const char *shard_account_boc, const char *message_boc) {
  ...
  auto result = emulator->emulate_transaction_sbs(std::move(account), message_cell, now, 0, block::transaction::Transaction::tr_ord);
  if (result.is_error()) {
    ERROR_RESPONSE(PSTRING() << "Emulate transaction failed: " << result.move_as_error());
  }
...
const char *transaction_emulator_sbs_result(void *transaction_emulator) {
  ...
  auto result = emulator->result_sbs();
  if (result.is_error()) {
    ERROR_RESPONSE(PSTRING() << "Emulate transaction failed: " << result.move_as_error());
  }
...
const char *transaction_emulator_sbs_get_stack(void *tvm_emulator) {
  ...
  if (!emulator->vm_sbs().get_stack().serialize(stack_cb)) {
...
int transaction_emulator_sbs_get_cont_distinguisher(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TransactionEmulator *>(tvm_emulator);

  return emulator->vm_sbs().get_cont_distinguisher();
}

void transaction_emulator_sbs_set_cont_distinguishers(void *tvm_emulator, int distinguisher, int distinguisher_true, int distinguisher_false) {
  auto emulator = static_cast<emulator::TransactionEmulator *>(tvm_emulator);

  emulator->vm_sbs().set_cont_distinguishers(distinguisher, distinguisher_true, distinguisher_false);
}

bool transaction_emulator_sbs_get_cont_distinguisher_triggered(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TransactionEmulator *>(tvm_emulator);

  return emulator->vm_sbs().get_cont_distinguisher_triggered();
}

void transaction_emulator_sbs_set_try_params(void *tvm_emulator, int primed, int triggered) {
  auto emulator = static_cast<emulator::TransactionEmulator *>(tvm_emulator);

  emulator->vm_sbs().set_try_params(primed, triggered);
}

int transaction_emulator_sbs_get_triggered_try_param(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TransactionEmulator *>(tvm_emulator);

  return emulator->vm_sbs().get_triggered_try_param();
}
...
const char *transaction_emulator_sbs_get_code_pos(void *tvm_emulator) {
  ...
  rs << emulator->vm_sbs().get_code()->get_base_cell()->get_hash().to_hex() << ":" << emulator->vm_sbs().get_code()->cur_pos();
...
const char *tvm_emulator_sbs_get_stack(void *tvm_emulator) {
  ...
  vm::CellBuilder stack_cb;
  if (!emulator->smc_.sbs_vm.get_stack().serialize(stack_cb)) {
    ERROR_RESPONSE(PSTRING() << "Couldn't serialize stack");
  }
  auto result_stack_boc = cell_to_boc_b64(stack_cb.finalize());
  if (result_stack_boc.is_error()) {
...
int tvm_emulator_sbs_get_cont_distinguisher(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TvmEmulator *>(tvm_emulator);

  return emulator->smc_.sbs_vm.get_cont_distinguisher();
}

void tvm_emulator_sbs_set_cont_distinguishers(void *tvm_emulator, int distinguisher, int distinguisher_true, int distinguisher_false) {
  auto emulator = static_cast<emulator::TvmEmulator *>(tvm_emulator);

  emulator->smc_.sbs_vm.set_cont_distinguishers(distinguisher, distinguisher_true, distinguisher_false);
}

bool tvm_emulator_sbs_get_cont_distinguisher_triggered(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TvmEmulator *>(tvm_emulator);

  return emulator->smc_.sbs_vm.get_cont_distinguisher_triggered();
}

void tvm_emulator_sbs_set_try_params(void *tvm_emulator, int primed, int triggered) {
  auto emulator = static_cast<emulator::TvmEmulator *>(tvm_emulator);
  emulator->smc_.sbs_vm.set_try_params(primed, triggered);
}

int tvm_emulator_sbs_get_triggered_try_param(void *tvm_emulator) {
  auto emulator = static_cast<emulator::TvmEmulator *>(tvm_emulator);
  return emulator->smc_.sbs_vm.get_triggered_try_param();
}
...
const char* tvm_emulator_sbs_get_code_pos(void *tvm_emulator) {
  ...
  std::ostringstream rs;
  rs << emulator->smc_.sbs_vm.get_code()->get_base_cell()->get_hash().to_hex() << ":" << emulator->smc_.sbs_vm.get_code()->cur_pos();
