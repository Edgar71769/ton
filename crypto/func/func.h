...
struct DebugInfo {
  ...
  bool ret = false;
  int try_catch_context_id = -1;
  bool is_try_end = false;
  bool want_vars = false;
  int context_id;
  int require_context_id;
  int branch_true_context_id = -1;
  int branch_false_context_id = -1;
};
...
extern std::deque<DebugInfo> debug_infos; // requires no reference/pointer invalidation on push, therefore deque not vector
extern int debug_info_context_id;
